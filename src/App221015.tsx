...
export default function App(){
    ...
    return <div>
        <div style={ { width:'100%', height:'300px', overflowY:'scroll', border:'1px solid red' } }>
            {/* { mode === 1 && <Markdown>{ value }</Markdown> }
            { mode === 2 && <MarkdownLab>{ value }</MarkdownLab> } */}
            <MarkdownLab>{ value }</MarkdownLab>
        </div>
        <textarea
            style={ { width:'100%', height:'300px', overflowY:'scroll', resize:'vertical' } }
            value={value}
